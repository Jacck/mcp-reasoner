...
export class GPROMCTS extends PolicyGuidedMCTS {
  ...
  private async getAlternativePaths(
    node: GPROPolicyNode,
  ): Promise<ThoughtNode[][]> {
    ...
    for (const [depth, nodes] of nodesByDepth) {
      if (depth <= node.depth) {
        const altNodes = nodes.filter(
          (n) => !currentPath.some((p) => p.id === n.id) && n.score > 0.5,
        );
  ...
  private async getBestOutcomes(node: GPROPolicyNode): Promise<string[]> {
    const allNodes =
      (await this.stateManager.getAllNodes()) as GPROPolicyNode[];
    return allNodes
  ...
  private async suggestImprovements(node: GPROPolicyNode): Promise<string[]> {
    ...
    if (node.valueEstimate < 0.5) {
      improvements.push(
        "Consider alternative approaches with higher value estimates",
      );
    }
  ...
  private async generateReasoningPrompt(
    ...
    return {
      ...
      context: {
        ...
        reasoning: path
          .map((n) => (n as GPROPolicyNode).reflections || [])
          .flat(),
      },
      reflection: {
        mistakes: mistakes,
  ...
  private constructPromptInstruction(
    node: GPROPolicyNode,
    mistakes: string[],
  ): string {
  ...
  public async processThought(
    ...
    const parentNode = request.parentId
      ? ((await this.getNode(request.parentId)) as GPROPolicyNode)
      : undefined;
  ...
  private async initializeGPRONode(
    node: GPROPolicyNode,
    parent?: GPROPolicyNode,
  ): Promise<void> {
    ...
    node.proximityScore = parent
      ? this.calculateProximityScore(node, parent)
      : 1.0;

    // Calculate KL divergence for trust region
    node.kl_div = parent ? this.calculateKLDivergence(node, parent) : 0;
    ...
    node.importanceSamplingRatio = parent
      ? this.calculateImportanceSamplingRatio(node, parent)
      : 1.0;
  }
  ...
  private calculateProximityScore(
    ...
    return Math.min(
      ratio * current.advantageEstimate,
      clipped * current.advantageEstimate,
    );
  }
  ...
  private calculateImportanceSamplingRatio(
    current: GPROPolicyNode,
    old: GPROPolicyNode,
  ): number {
    let totalRatio = 0;
  ...
  private async sampleTrajectories(
    node: GPROPolicyNode,
  ): Promise<GPROPolicyNode[]> {
  ...
  private async selectNextNode(
    node: GPROPolicyNode,
  ): Promise<GPROPolicyNode | null> {
  ...
  private async updateGPROPolicy(trajectories: GPROPolicyNode[]): Promise<{
    ...
    for (const node of trajectories) {
      ...
      const newValue =
        node.valueEstimate + this.learningRate * node.advantageEstimate;
  ...
  private updateGPROMetrics(trajectories: GPROPolicyNode[]): void {
    ...
    this.gproMetrics.advantageStats = {
      ...
      std: Math.sqrt(
        advantages.reduce((a, b) => a + b * b, 0) / advantages.length,
      ),
